...
const FlightPredictionForm = () => {
  const [formData, setFormData] = useState({
    ...
    WheelsOff: '',
    // ArrDel15: '',
    Airline: [], // Store one-hot encoded airline values in an array
    // Add other feature fields here
    Month: [],
    DayofMonth: [],
    DayOfWeek: [],
    OriginAirportID: [],
  ...
  const handleChange = (event) => {
    ...
    if (name === 'Airline') {
      // Handle the Airline dropdown selection for one-hot encoding
      const selectedAirlines = value ? [value] : []; // Store the selected airline(s) in an array
      setFormData({
        ...formData,
    ...
    if (name === 'Month') {
        // Handle the Month dropdown selection for one-hot encoding
        const selectedMonths = value ? [value] : []; // Store the selected Month(s) in an array
        setFormData({
          ...formData,
    ...
    if (name === 'DayofMonth') {
        // Handle the DayofMonth dropdown selection for one-hot encoding
        const selectedDayofMonths = value ? [value] : []; // Store the selected DayofMonth(s) in an array
        setFormData({
          ...formData,
          DayofMonth: selectedDayofMonths,
        });
      } else {
        // Handle other input field changes
        setFormData({
    ...
    if (name === 'DayOfWeek') {
        // Handle the DayOfWeek dropdown selection for one-hot encoding
        const selectedDayOfWeeks = value ? [value] : []; // Store the selected DayOfWeek(s) in an array
        setFormData({
          ...formData,
          DayOfWeek: selectedDayOfWeeks,
        });
      } else {
        // Handle other input field changes
        setFormData({
    ...
    if (name === 'OriginAirportID') {
        // Handle the OriginAirportID dropdown selection for one-hot encoding
        const selectedOriginAirportIDs = value ? [value] : []; // Store the selected OriginAirportID(s) in an array
        setFormData({
          ...formData,
          OriginAirportID: selectedOriginAirportIDs,
        });
      } else {
        // Handle other input field changes
        setFormData({
    ...
    if (name === 'DestAirportID') {
        // Handle the DestAirportID dropdown selection for one-hot encoding
        const selectedDestAirportIDs = value ? [value] : []; // Store the selected DestAirportID(s) in an array
        setFormData({
          ...formData,
          DestAirportID: selectedDestAirportIDs,
        });
      } else {
        // Handle other input field changes
        setFormData({
  ...
  return (
    <form onSubmit={handleSubmit}>
      {/* Input fields for non-one-hot-encoded features */}
      <label>
        Departure Time:
        <input
          type="text"
      ...
      <label>
        ...
        <input
          type="text"
          name="WheelsOff"
          value={formData.WheelsOff}
          onChange={handleChange}
        />
      </label>
      {/* Add similar fields for other non-one-hot-encoded features here */}

      {/* Dropdown for one-hot encoded Airline feature */}
      <label>Airline:</label>
      <select
        name="Airline"
        value={formData.Airline[0] || ''}
        onChange={handleChange}
      >
        <option value="">Select an airline</option>
        {airlines.map((airline) => (
          <option key={airline} value={airline}>
            {airline.replace('Airline_', '')}
          </option>
        ))}
      </select>
      ...
      <select
        ...
        value={formData.OriginAirportID[0] || ''}
        onChange={handleChange}
      >
        <option value="">Select Origin Airport ID</option>
        {originairportids.map((originalairportid) => (
          <option key={originalairportid} value={originalairportid}>
            {originalairportid.replace('OriginAirportID_', '')}
          </option>
        ))}
      </select>

      <label>Dest Airport ID:</label>
      <select
        name="DestAirportID"
        value={formData.DestAirportID[0] || ''}
        onChange={handleChange}
      >
        <option value="">Select Dest Airport ID</option>
        {destairportids.map((destairportid) => (
          <option key={destairportid} value={destairportid}>
            {destairportid.replace('DestAirportID_', '')}
          </option>
        ))}
      </select>
